...
import com.example.visibility_tracking.ui.theme.VisibilityTrackingTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Visibility Tracking 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_BasicVisibilityLogging()
            1 -> Practice2_AdImpressionTracking()
            2 -> Practice3_VideoAutoplay()
        }
    }
}
