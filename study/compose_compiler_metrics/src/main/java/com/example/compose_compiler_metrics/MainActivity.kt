...
import com.example.compose_compiler_metrics.ui.theme.ComposeCompilerMetricsTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Compiler Metrics 학습") }
            )
        }
    ) { innerPadding ->
        Column(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        Row(
            ...
            FilterChip(
                ...
                label = { Text("Report 분석") }
            )
        }

        when (selectedPractice) {
            0 -> Practice1_StableShoppingCart()
            1 -> Practice2_LambdaMemoization()
            2 -> Practice3_ReportAnalysis()
        }
    }
}
