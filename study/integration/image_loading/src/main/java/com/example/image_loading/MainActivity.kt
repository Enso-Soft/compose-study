...
import com.example.image_loading.ui.theme.ImageLoadingTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Image Loading (Coil) 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_BasicAsyncImage()
            1 -> Practice2_ImageGallery()
            2 -> Practice3_StateHandling()
        }
    }
}
