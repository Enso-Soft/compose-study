...
import com.example.animation_advanced.ui.theme.AnimationAdvancedTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Animation Advanced 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_ToggleButton()
            1 -> Practice2_BouncyCounter()
            2 -> Practice3_LoadingDots()
        }
    }
}
