...
import com.example.constraint_layout.ui.theme.ConstraintLayoutTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("ConstraintLayout 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_LoginForm()
            1 -> Practice2_BarrierSettings()
            2 -> Practice3_ChainNavigation()
        }
    }
}
