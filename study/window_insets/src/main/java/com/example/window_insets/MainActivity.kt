...
import com.example.window_insets.ui.theme.WindowInsetsTheme

class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()  // 중요! edge-to-edge 활성화
        setContent {
            WindowInsetsTheme {
                MainScreen()
            }
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_SafeDrawingPadding()
            1 -> Practice2_ImePadding()
            2 -> Practice3_ScaffoldWithList()
        }
    }
}
