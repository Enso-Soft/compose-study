...
import com.example.lifecycle_integration.ui.theme.LifecycleIntegrationTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Lifecycle Integration") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_ScreenTimeTracker()
            1 -> Practice2_SensorSimulator()
            2 -> Practice3_StateBasedUI()
        }
    }
}
