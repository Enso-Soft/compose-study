...
import com.example.intrinsic_measurements.ui.theme.IntrinsicMeasurementsTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Intrinsic Measurements 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_ProfileComparison()
            1 -> Practice2_ProductCards()
            2 -> Practice3_TableLayout()
        }
    }
}
