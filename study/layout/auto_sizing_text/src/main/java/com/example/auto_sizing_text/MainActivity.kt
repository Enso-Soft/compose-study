...
import com.example.auto_sizing_text.ui.theme.AutoSizingTextTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Auto-sizing Text 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_ProductCardScreen()
            1 -> Practice2_PriceBannerScreen()
            2 -> Practice3_DashboardScreen()
        }
    }
}
