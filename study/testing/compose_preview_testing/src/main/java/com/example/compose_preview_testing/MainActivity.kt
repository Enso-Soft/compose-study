...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Compose Preview Testing 학습") }
            )
        }
    ) { innerPadding ->
        Column(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        Row(
            ...
            FilterChip(
                ...
                label = { Text("Multipreview") }
            )
        }

        when (selectedPractice) {
            0 -> Practice1_BasicProvider()
            1 -> Practice2_CollectionProvider()
            2 -> Practice3_MultipreviewCombination()
        }
    }
}
