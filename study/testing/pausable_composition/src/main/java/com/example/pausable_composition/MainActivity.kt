...
import com.example.pausable_composition.ui.theme.PausableCompositionTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Pausable Composition 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_JankMeasurement()
            1 -> Practice2_PrefetchMonitoring()
            2 -> Practice3_ComplexityComparison()
        }
    }
}
