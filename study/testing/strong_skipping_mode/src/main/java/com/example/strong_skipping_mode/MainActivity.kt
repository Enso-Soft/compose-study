...
import com.example.strong_skipping_mode.ui.theme.StrongSkippingModeTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Strong Skipping Mode 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_StableUnstableScreen()
            1 -> Practice2_SkipVerificationScreen()
            2 -> Practice3_LambdaMemoizationScreen()
        }
    }
}
