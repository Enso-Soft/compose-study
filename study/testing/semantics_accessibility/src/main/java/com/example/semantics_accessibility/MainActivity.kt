...
import com.example.semantics_accessibility.ui.theme.SemanticsAccessibilityTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Semantics & Accessibility") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_ProfileCard()
            1 -> Practice2_AuctionTimer()
            2 -> Practice3_FormTraversal()
        }
    }
}
