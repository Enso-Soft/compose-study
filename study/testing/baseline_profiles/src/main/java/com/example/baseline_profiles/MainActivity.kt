...
import com.example.baseline_profiles.ui.theme.BaselineProfilesTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Baseline Profiles 학습") }
            )
        }
    ) { innerPadding ->
        Column(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        Row(
            ...
            FilterChip(
                ...
                label = { Text("ReportDrawn") }
            )
        }

        when (selectedPractice) {
            0 -> Practice1_ProfileRules()
            1 -> Practice2_ProfileGenerator()
            2 -> Practice3_ReportDrawn()
        }
    }
}
