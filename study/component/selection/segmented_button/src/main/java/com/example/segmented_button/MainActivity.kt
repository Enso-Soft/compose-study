...
import com.example.segmented_button.ui.theme.SegmentedButtonTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Segmented Button 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Exercise1_ViewModeSelector()
            1 -> Exercise2_MultiFilter()
            2 -> Exercise3_SettingsScreen()
        }
    }
}
