...
import com.example.material_symbols.ui.theme.MaterialSymbolsTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Material Symbols 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_BasicIcon()
            1 -> Practice2_StyleGallery()
            2 -> Practice3_DynamicTheme()
        }
    }
}
