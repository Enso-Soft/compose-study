...
import com.example.permission_handling.ui.theme.PermissionHandlingTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Permission Handling 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_LocationPermission()
            1 -> Practice2_NotificationWithSettings()
            2 -> Practice3_MultiplePermissions()
        }
    }
}
