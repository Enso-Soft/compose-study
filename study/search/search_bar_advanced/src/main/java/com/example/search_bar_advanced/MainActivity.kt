...
import com.example.search_bar_advanced.ui.theme.SearchBarTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Material3 SearchBar 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_BasicSearchBar()
            1 -> Practice2_SearchHistory()
            2 -> Practice3_AdvancedSearch()
        }
    }
}
