...
import com.example.desktop_extensions.ui.theme.DesktopExtensionsTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Desktop Extensions 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_MenuBarScreen()
            1 -> Practice2_TrayScreen()
            2 -> Practice3_IntegrationScreen()
        }
    }
}
