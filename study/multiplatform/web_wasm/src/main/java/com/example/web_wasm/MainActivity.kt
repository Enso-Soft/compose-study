...
import com.example.web_wasm.ui.theme.WebWasmTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Compose for Web (Wasm) 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_StructureScreen()
            1 -> Practice2_ConversionScreen()
            2 -> Practice3_InteropScreen()
        }
    }
}
