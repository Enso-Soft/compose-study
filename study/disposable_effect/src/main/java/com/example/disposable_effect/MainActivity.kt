...
import com.example.disposable_effect.ui.theme.DisposableEffectTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("DisposableEffect 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_LifecycleScreen()
            1 -> Practice2_BackgroundTimerScreen()
            2 -> Practice3_UserSubscriptionScreen()
        }
    }
}
