...
import com.example.navigation_drawer.ui.theme.NavigationDrawerTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Navigation Drawer 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_BasicDrawer()
            1 -> Practice2_IconsAndSelection()
            2 -> Practice3_CompleteDrawer()
        }
    }
}
