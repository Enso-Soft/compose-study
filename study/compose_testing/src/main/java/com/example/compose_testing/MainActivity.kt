...
import com.example.compose_testing.ui.theme.ComposeTestingTheme
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Compose UI Testing 학습") }
            )
        }
    ) { innerPadding ->
        Column(
...
fun PracticeNavigator() {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        when (selectedPractice) {
            0 -> Practice1_FinderBasics()
            1 -> Practice2_ActionsAndState()
            2 -> Practice3_ListAndInput()
        }
    }
}
